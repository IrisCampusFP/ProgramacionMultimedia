...
//        // Register a password for that user
            ...
            public void onClick(View v) {
                ...
                usuarios.get().addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        // Iterate over all user IDs to find one whose 'nombre' key matches the entered name
                        for (DataSnapshot refUsuario : task.getResult().getChildren()) {
                            ...
                            // If the entered username matches the one in the database
                            if (entradaNombreUsuario.equals(nombreUsuarioBD)) {
                                usuarioEncontrado = true;

                                // 4. Se obtiene la clave del usuario
                                // 4. Obtain the user's password
                                ...
                                // 5. If the password entered by the user matches the one in the database
                                if (entradaClaveUsuario.equals(claveUsuarioBD)) {

                                    // Obtengo los puntos y el nivel del usuario
                                    // Get the user's points and level
                                    ...
                                    // Inform the user that the session started successfully
                                    ...
                                    // Upon login, redirect the user to the game
                        ...
                        // If it fails to connect to the database
                    } else {
